    ...
    def prepare_lm_input_target(self, sos_emb, text_token, text_token_emb, text_token_len, task_id_emb, speech_token, speech_token_emb, speech_token_len, instruct_token=None, instruct_token_emb=None, instruct_token_len=None):
        ...
        for i in range(len(text_token)):
            ...
            # unistream sequence
                ...
                instruct_len = instruct_token_len[i] if instruct_token_len is not None else 0
                instruct_emb = instruct_token_emb[i] if instruct_token_emb is not None else torch.zeros(0, self.llm_input_size)
                this_lm_target = torch.tensor([IGNORE_ID] * (1 + instruct_len + text_token_len[i]) + speech_token[i].tolist() + [self.eos_token])
                this_lm_input = torch.concat([sos_emb.squeeze(dim=0), instruct_emb, text_token_emb[i], task_id_emb.squeeze(dim=0), speech_token_emb[i]], dim=0)
